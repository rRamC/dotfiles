...
| -------|---------|---------------|
| `GH_USERNAME` | The github username | "rramc" |
...
| `KEYBASE_USERNAME` | Keybase username | "rramc" |
...
1. **_Only in this README file and `files/.zprofile` files (and nowhere else):_** Find and replace the strings that reference my usernames to your equivalent ones (for eg, you can search for `rramc` and `rramc` and replace them with your values).
...
```zsh
export GH_USERNAME="rramc"; export DOTFILES_DIR="${HOME}/.bin-oss"; curl -fsSL "https://raw.githubusercontent.com/${GH_USERNAME}/dotfiles/master/scripts/fresh-install-of-osx.sh" | zsh
```
